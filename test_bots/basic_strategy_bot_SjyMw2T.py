"""
Basic strategy bot using hand strength evaluation.
Folds if cost too high relative to hand strength,
otherwise calls or raises based on hand strength.
"""
from bots.base import CountingBot
from bots.hand_evaluator import HandEvaluator
...
class Bot(CountingBot):
    ...
    # Higher = more aggressive betting for same hand strength
    AGGRESSION_BY_STREET = {
        'preflop': 0.6,    # More cautious preflop
        'flop': 0.7,
        'turn': 0.8,
        'river': 0.9       # More aggressive on river
    }
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        try:
            ...
            # Determine max amount willing to call/raise based on hand strength
            aggression = self.AGGRESSION_BY_STREET.get(street, 0.7)
            max_call_ratio = hand_strength * aggression  # 0-0.9 of stack typically
            max_call_amount = my_stack * max_call_ratio
            
            # Decision logic
            # If we need to call and it's affordable relative to hand strength
            if current_call_amount <= max_call_amount:
                # Raise if hand is strong enough
            ...
            # On any error, fallback to calling
            return self._execute_call(valid_actions)
    ...
    def _can_raise(self, valid_actions):
        """Check if raise action is available."""
    ...
    def _calculate_raise_amount(self, hand_strength, current_bet, stack):
        ...
        if hand_strength < 0.7:
            return current_bet * 2  # Bet 2x
        elif hand_strength < 0.85:
            return current_bet * 3  # Bet 3x
        else:
            return int(stack * 0.3)  # Go bigger with strong hands
    
    def _get_raise_action(self, valid_actions, amount):
        """Get a raise action with specified amount as tuple (action, amount)."""
    ...
    def _execute_call(self, valid_actions):
        """Execute call action, return tuple (action, amount)."""
        call_action = next((a for a in valid_actions if a['action'] == 'call'), None)
        if call_action:
